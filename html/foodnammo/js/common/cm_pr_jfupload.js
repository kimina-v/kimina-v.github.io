...
const prjfupload = {
	...
	initImageUpload : function (p_opt) {
		...
		target.fileupload({
			...
			, pasteZone: null
		}).on("fileuploadadd", function (e, data) { // 파일 업로드 부분

			prjfupload.printLog("fileuploadadd");
        	...
        	data.submit()
	        	.done (function (result, textStatus, jqXHR) {
					...
					if (typeof options.success == "function") {
						console.log("업로드 성공 : " + JSON.stringify(imgList) );
						if(options.singleFileUploads){ // 멀티 모드
							options.success(imgList[0], options.uploadCd, data); // 실제 src를 그리는 부분
						}else{
							options.success(imgList, options.uploadCd, data); // 실제 src를 그리는 부분
						}
					}

	        		if (!options.isSelfMakeTag) {
						prjfupload.makeImageUploadTag(imgList, options.uploadCd, options.formName, "R");
	        		}
	        	})
			...
			prjfupload.printLog("fileuploadprocessalways");
	...
	deleteImage : function (uploadCd, formName , imgId) {
		if(imgId){
			const div = jQuery("#div_"+uploadCd+"_"+imgId);
			const flagAction = div.find("input[name*='image_action_type']").val();
			console.log("그리기 ㅋㅋㅋㅋ :" );
			if (flagAction==="M") {
			...
			jQuery("#"+uploadCd+"_preview_"+imgId).remove();
			jQuery("#"+uploadCd+"_upload_name_"+imgId).remove();
			div.remove();
		}else{
			console.log("그리기222 ㅋㅋㅋㅋ :" );
			const divList = jQuery("[id^='div_"+uploadCd+"']");
			...
			console.log("그리기 ㅋㅋㅋㅋ444444 :" );
			jQuery("#"+uploadCd+"_preview").empty();
			jQuery("#"+uploadCd+"_upload_name").empty();
			divList.remove();
		}
