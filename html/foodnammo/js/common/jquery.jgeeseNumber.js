...
(function ($, G) {
    ...
    $.extend(K.prototype, {
        ...
        _attachFloat: function (p_target, p_opt) {
            $(p_target).focus(this._doFocus).keydown(this._doKeydown).keyup(this._doKeyup).blur(this._doBlur);
        ...
        _setNumber: function (target) {
            ...
            if (arrVal.join("") == "" && arrVal2.join("") == "") {
                ...
                target.val(arrVal.join("") + arrVal2.join(""));
            } else {
                target.val(parseInt("0" + arrVal.join(""), 10) + arrVal2.join(""));
            }
        },
        _getOS: function () {
    ...
    $.fn.jgeeseFloat = function (p_opt) {
        ...
        return this.each(function () {
            $.jgeeseNumber._attachFloat(this, p_opt);
        });
    };

    $.jgeeseNumber = new K;
    $.jgeeseNumber.initialized = false;
    $.jgeeseNumber.version = "1.0";
})(jQuery);
 ...
 * @param p_opt
 */
function addOnlyFloatEvent(target, p_opt) {

    if (target == null || typeof target != "object")
        target = jQuery(".onlyFloat");

    target.jgeeseFloat(p_opt)
        .css({
            "ime-mode": "disabled"
        });
}